...
Les TIC facilitent l’accès à l’information, améliorent la qualité du travail collaboratif, et permettent de réaliser des simulations complexes dans des environnements spécialisés.
...
Son utilité dans les EDP réside dans sa capacité à implémenter facilement des schémas numériques (différences finies, éléments finis, etc.) et à produire des graphiques clairs et précis.
...
Il constitue une alternative gratuite et très performante à MATLAB.

\subsubsection{GeoGebra}
GeoGebra est un outil interactif permettant la visualisation géométrique et mathématique.  
Bien que moins utilisé pour des simulations avancées, il sert à illustrer des concepts liés aux EDP, aux conditions limites, ou à la propagation d’une solution.
...
C’est un outil idéal pour l’enseignement et la démonstration de méthodes numériques.
...
Elle permet un travail collaboratif en temps réel, ce qui facilite la rédaction de rapports, d’articles ou de mémoires scientifiques.

\subsubsection{Google Colab}
Google Colab est un environnement cloud permettant d'exécuter du code Python sans installation locale.  
Il est utilisé pour les calculs, les simulations et le partage des notebooks entre étudiants et chercheurs.

\subsubsection{Zotero}
Zotero est un logiciel de gestion bibliographique permettant de collecter, organiser et citer automatiquement les références dans un document LaTeX, Word ou autres.
...
Dans le cadre des EDP et des simulations numériques, GitHub permet de sauvegarder le code MATLAB/Python, suivre les modifications et travailler efficacement en groupe.
...
Elle est utile pour concevoir des affiches scientifiques, posters et diapositives à forte dimension visuelle.
...
Cette équation représente un phénomène de diffusion progressive de la chaleur dans une barre de longueur $L$, d'extrémités non chauffés ou de température constante considéré nulle, et de température $f(x)$ au temps $t=0$. %ICI
...
\]
reste strictement inférieur à $0{,}5$ pour toutes les simulations, garantissant ainsi la stabilité de la méthode.
...
% --- Solution exacte sur la même grille x ---
u_exact = exp(-alpha*pi^2*T) .* sin(pi*x);
...
Cette erreur est considérée négligeable. La méthode numérique assure donc une bonne approximation de la solution exacte.

\pagebreak
\section{Stockage des codes LaTeX \& MATLAB sur GitHub}
\label{sec:GITHUB}
Pour assurer une gestion efficace du code et des résultats, un dépôt GitHub a été créé. Cela permet de centraliser tous les fichiers du projet, d'assurer la traçabilité des modifications et de faciliter le partage avec les collaborateurs.

Le dépôt est structuré de manière claire :

\begin{tcolorbox}[colback=gray!5, colframe=black, width=0.9\textwidth]
\begin{verbatim}
/ProjetTIC
/Code MATLAB             # Scripts MATLAB
/Code LaTeX              # Fichier main.tex du PDF et Beamer
/Figures                 # Graphiques
/Rapports                # Fichiers PDF et Beamer
\end{verbatim}
\end{tcolorbox}
